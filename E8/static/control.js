...
function fgen(i, add) {
  return function() {
    add ? points[i].push({}) : points[i].pop();
  };
}
...
function setup() {
  ...
  for (i = 0; i < layers; i++) {
    ...
    rots.push(createSlider(-0.05, 0.05, 0, 0.0005));
    rots[i].position(130, 30 * i + 50);
    rot.push(0);
    rots[i].elt.oninput = function(){d = {'layer': i, 'value': this.value}; send_data(d)}
    // rots[i].elt.oninput = function(){send_data({i:this.value()})}
    ...
    bright.push(createSlider(0, 100, 50 + (50 / layers) * i, 0.1));
    bright[i].position(300, 30 * i + 250);
  }
  ups = [];

  wiggle.push(createSlider(0, 4, 0, 0.1));
  wiggle.push(createSlider(0, 4, 0, 0.01));
  wiggle[0].position(300, 490);
  wiggle[1].position(300, 520);

  radii.push(createSlider(-0.005, 0.005, 0, 0.00005)); // oscilation in radius x
  radii.push(createSlider(-0.05, 0.05, 0, 0.0005)); // offset per layer x
  radii.push(createSlider(-0.005, 0.005, 0, 0.00005)); // oscilation in radius y
  radii.push(createSlider(-0.05, 0.05, 0, 0.0005)); // offset per layer y
